...
export function HabitDay({ defaultCompleted = 0, amount = 0, date }: HabitProps) {
  ...
  const handleCompleteChanged = (completed: number) => {
    ...
    <Popover.Root>
      ...
      <Popover.Portal>
        <Popover.Content className="min-w-[320px] p-6 rounded-2xl bg-zinc-900 flex flex-col">
          ...
          <HabitsList date={date} onCompletedChanged={handleCompleteChanged}/>

          <Popover.Arrow height={16} width={8} className="fill-zinc-900" />
        </Popover.Content>
      </Popover.Portal>
    </Popover.Root>
  );
